# -*- coding: utf-8 -*-
"""Puter Bot Service channel plugin entry point."""
...
class PuterBotChannelPlugin:
    """Puter Bot Service channel plugin."""
    ...
    def register(self, api: PluginApi):
        ...
        """
        from .channel import PuterChannel
        from .constants import (
            PUTER_ORIGIN_URL, PUTER_API_URL, DEFAULT_HTTP_URL
        )

        api.register_channel(
            channel_class=PuterChannel,
            label="Puter Bot",
            description="Puter Bot Service (Bot Framework) integration",
            icon=(
            ...
            config_fields=[
                ...
                },
                {
                    ...
                    "label": {
                        "zh-CN": "话题对话中需要 @提及",
                        "en-US": "Thread Require @Mention",
                    },
                    "type": "switch",
                    "required": False,
                    "default": False,
                    "help": {
                        "zh-CN": "开启后，话题对话中仅在被 @提及 时才会回复",
                        "en-US": (
                            "When enabled, bot only responds in thread "
                            "chats when explicitly @mentioned"
                        ),
                        "ja-JP": (
                            "有効にすると、グループチャットでは" "明示的に@メンションされた場合のみ応答します"
        ...
        logger.info("✓ Puter Bot channel registered")


# Export plugin instance
plugin = PuterBotChannelPlugin()
